...
* Investigate how to add automatic Spin Up which starts the DataCruch VM.
* Investigate how to add automatic shut down after certain time for the DataCrunch VM
* Liqo is set up for k3s clusters now. Invetigate if there is need for k8s. If the k3s doesn't have enough resources then modify the insturctions to be compatible with k8s.
